...
3. ✅ **Insider-aware / user-pivoted correlation — SHIPPED (ML-4).** The similarity graph excludes the user pivot by design (avoids benign drag in the external-C2 IT case); `PRAHARI_INSIDER_FUSION=1` adds it when there is no external anchor, lifting scenario-2 insider fusion recall **62%→69%** and consolidating the campaign into one incident, with **zero** scenario-1 regression (13/13 held). Next: make the switch *automatic* (detect the missing external anchor per-incident) rather than a flag. Details: `RESULTS.md` §7.
