...
export function StoryLens({
  ...
  const fill = Math.min(96, (playDay / M.dmax) * 96);
  ...
  return (
    <div style={{ padding: "26px 6px 8px" }}>
      <div style={{ position: "relative", padding: "0 2px" }}>
        <div style={{ position: "absolute", left: "2%", right: "2%", top: 9, height: 4, background: "#EEF2F6", borderRadius: 3 }} />
        <div style={{ position: "absolute", left: "2%", top: 9, height: 4, background: "linear-gradient(90deg,#D97706,#DC2626)", borderRadius: 3, maxWidth: "96%", width: `${fill}%` }} />
        <div style={{ display: "flex", justifyContent: "space-between", gap: 8, position: "relative" }}>
          {M.stations.map((st) => {
            ...
            return (
              <div key={st.id} style={{ flex: "1 1 0", display: "flex", flexDirection: "column", alignItems: "center", textAlign: "center", minWidth: 0, opacity: lit ? 1 : 0.7, transition: "opacity .35s" }}>
                <div style={{ width: 20, height: 20, borderRadius: "50%", background: lit ? hc : "#fff", border: `3px solid ${lit ? hc : "#D8E0E8"}`, boxShadow: lit ? `0 0 0 4px ${hc === "#059669" ? "rgba(5,150,105,0.14)" : hc === "#DC2626" ? "rgba(220,38,38,0.12)" : "rgba(217,119,6,0.12)"}` : "none", transition: "all .35s ease" }} />
                <div className={s.mono} style={{ fontSize: 13, fontWeight: 700, marginTop: 12, color: lit ? "var(--ink)" : "var(--mut)", transition: "color .35s" }}>{st.id}</div>
                <div style={{ fontSize: 11, fontWeight: 600, marginTop: 3, lineHeight: 1.25, color: lit ? "var(--ink)" : "var(--mut)", textDecoration: st.prevented ? "line-through" : "none", textDecorationColor: "#DC2626", textDecorationThickness: 1.5 }}>
                  {st.name}
                </div>
                <div style={{ fontSize: 9.5, color: "var(--mut)", marginTop: 2 }}>{st.tactic}</div>
                <div className={s.mono} style={{ fontSize: 9.5, color: "var(--mut)", marginTop: 4 }}>
                  {st.host} · {st.date}
                </div>
                {st.verdict && lit && (
                  <div style={{ marginTop: 8, fontSize: 9.5, fontWeight: 700, letterSpacing: "0.03em", color: st.prevented ? "#991B1B" : "#065F46", background: st.prevented ? "rgba(220,38,38,0.07)" : "rgba(5,150,105,0.1)", border: `1px solid ${st.prevented ? "rgba(220,38,38,0.28)" : "rgba(5,150,105,0.3)"}`, borderRadius: 7, padding: "3px 8px", whiteSpace: "nowrap" }}>
                    {st.prevented ? "✕ " : "✓ "}
                    {st.verdict}
                  </div>
                )}
      ...
      <div style={{ fontSize: 11.5, color: "var(--mut)", textAlign: "center", marginTop: 22 }}>
        Scrub the clock above — each station ignites the moment the playhead crosses its first-observed time.
      </div>
      <div style={{ marginTop: 22, borderTop: "1px solid var(--line2)", paddingTop: 6 }}>
        {M.stations.map((st) => (
...
export function PathLens({ model: M }: { model: ConsoleModel }) {
  ...
  return (
    <div style={{ paddingTop: 22, display: "flex", flexDirection: "column", gap: 10, maxWidth: 860, margin: "0 auto" }}>
      <div style={{ display: "flex", alignItems: "center", gap: 12, background: "#FBFCFD", border: "1px solid var(--line2)", borderRadius: 13, padding: "13px 16px" }}>
      ...
      {M.hops.map((h, i) => (
        <div key={i} style={{ display: "flex", alignItems: "stretch", gap: 12, marginLeft: 15 }}>
          ...
          <div style={{ flex: 1, background: "#fff", border: "1px solid var(--line2)", borderRadius: 13, padding: "12px 16px", display: "flex", alignItems: "center", gap: 16, flexWrap: "wrap" }}>
...
export function EventsLens({ model: M, onOpen }: { model: ConsoleModel; onOpen: (id: string) => void }) {
  ...
  return (
    <div style={{ paddingTop: 14 }}>
      ...
      <div className={s.thead} style={{ display: "grid", gridTemplateColumns: grid, padding: "0 12px 10px" }}>
        <div>#</div>
        <div>Event</div>
      ...
      {shown.map((e, i) => {
        const v = verd(e);
        return (
          <button key={e.id} className={s.rowBtn} onClick={() => onOpen(e.id)} style={{ display: "grid", gridTemplateColumns: grid, alignItems: "center", padding: "11px 12px" }}>
            <div className={s.mono} style={{ fontSize: 12, color: "var(--mut)", fontWeight: 600 }}>{String(i + 1).padStart(2, "0")}</div>
            <div className={s.mono} style={{ fontSize: 11.5 }}>{e.evt}</div>
            ...
            <div style={{ paddingLeft: 16, display: "flex", flexDirection: "column" }}>
              <span className={s.mono} style={{ fontSize: 11.5, fontWeight: 600 }}>{e.tech ?? e.type}</span>
              <span style={{ fontSize: 10.5, color: "var(--mut)" }}>{e.techName}</span>
            </div>
            ...
            <div style={{ textAlign: "right" }}>
              <span style={{ fontSize: 9.5, fontWeight: 700, letterSpacing: "0.03em", color: v.c, background: v.bg, borderRadius: 5, padding: "2px 7px" }}>{v.t}</span>
            </div>
          </button>
        );
      })}
