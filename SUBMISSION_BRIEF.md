...
2. **The live Claude attribution agent has been run end-to-end** (2026-07-04) on both the controlled APT and the held-out insider scenario — through the Claude Code **subscription CLI** (`make attribute-agent-live`), so **no `ANTHROPIC_API_KEY` was required**. Full tool-use investigation (6–9 calls), RAG-cited, ground-truth-blind; transcripts in `docs/LIVE_AGENT_RUN.md`. The reproducible attribution *number* stays the deterministic mapper (92.3%); the agent adds narrative + next-move prediction (superseding the earlier "single live run pending").
