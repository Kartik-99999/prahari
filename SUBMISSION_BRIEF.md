...
2. **The live Claude attribution agent runs end-to-end** (2026-07-04/05) on both scenarios through the Claude Code **subscription CLI** (`make attribute-agent-live`), so **no `ANTHROPIC_API_KEY` was required** — a real 6–9-call tool-use investigation. **Scored against ground truth (`make score-agent`):** first scoring caught it citing **benign** events (0 malicious — a truncation/ordering bug in the incident-events tool); after the fix (rank incident events by `anomaly_score`, raise the result budget), it grounds on the **malicious** events — scenario-1 6/6 GT techniques + 11 per-event correct, scenario-2 **20 per-event correct vs the deterministic mapper's 2/45** (T1005 data-theft 18/18). The **92.3% mapper stays the stable reproducible number**; the agent measurably beats it on the hard insider case. Transcripts + before/after scoring in `docs/LIVE_AGENT_RUN.md`. (Supersedes "single live run pending"; an interim overclaim that the agent "recovered the 2/45 gap" was caught by scoring, retracted, then earned back by the fix.)
