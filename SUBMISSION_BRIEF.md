...
2. **The live Claude attribution agent runs end-to-end** (2026-07-04/05) on both scenarios through the Claude Code **subscription CLI** (`make attribute-agent-live`), so **no `ANTHROPIC_API_KEY` was required** — a real 6–9-call tool-use investigation. **Scored against ground truth (`make score-agent`):** first scoring caught it citing **benign** events (0 malicious — a truncation/ordering bug in the incident-events tool); after the fix (rank incident events by `anomaly_score`, raise the result budget), it **reliably grounds on the malicious events** — scenario-1 6/6 GT techniques + 17/21 grounded; scenario-2 14–24 of ~25 citations malicious vs the mapper's ~2/45. The *exact* ATT&CK label on adjacent techniques (T1005⇄T1039) varies run-to-run (a favourable run hit 20 exact), so the **92.3% mapper stays the stable reproducible number** and grounding is the robust win. Transcripts + before/after + variance in `docs/LIVE_AGENT_RUN.md`. (Supersedes "single live run pending"; an interim overclaim that the agent "recovered the 2/45 gap" was caught by scoring and retracted, then a second overclaim — a hard "20-vs-2" — was corrected to the robust grounding metric after a re-run showed the exact-match is noisy.)
