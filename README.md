...
2. The live Claude attribution agent **runs end-to-end** on both scenarios via the Claude Code **subscription CLI** (no `ANTHROPIC_API_KEY` needed) — a real multi-call tool-use investigation. But we **scored it against ground truth** and report the result straight: it names a mostly-correct technique *set* (4/6 distinct techniques each scenario) yet its per-event evidence **citations land on benign context events, not the malicious ones** — so the reproducible attribution *number* remains the **deterministic mapper (92.3%)**, and closing the agent's citation-grounding gap is the top follow-up. Full scoring + root cause: [`docs/LIVE_AGENT_RUN.md`](docs/LIVE_AGENT_RUN.md).
