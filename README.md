...
2. The live Claude attribution agent has been **run end-to-end** on both the controlled APT and the held-out insider scenario — via the Claude Code **subscription CLI** (no `ANTHROPIC_API_KEY` needed), same tools + cite-or-abstain contract. Transcripts: [`docs/LIVE_AGENT_RUN.md`](docs/LIVE_AGENT_RUN.md). The reproducible attribution *number* remains the deterministic mapper (92.3%); the agent adds RAG-cited narrative + next-move prediction.
